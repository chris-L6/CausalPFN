...
class DiscreteCausalPFN:
    ...
    def predict_epos(
        ...
        Args: 
            ...
            take_mean (bool | str): Whether or not to take the mean of all k EPOs estimated for each treatment value
            alpha (float | None): Confidence level for estimating CI. If None, do not estimate CI.
        """
        # Consistency checks
        ...
        if len(np.unique(T)) != len(discrete_treatment_vals) or not np.allclose(np.sort(np.unique(T)), discrete_treatment_vals, rtol=1e-10, atol=1e-5):
        ...
        if (take_mean in [False, "both"]) and alpha:
            raise ValueError("Incompatible take_mean and alpha values (I haven't implemented that).")
        ...
        # If alpha is not None, also record the samples for the CI calculation
        ...
        for bin_pair in bin_pairs:
            ...
            if not alpha:
                ...
                mu_0, mu_1, samples_0, samples_1 = self._predict_mu_0_and_mu_1(X_temp, T_temp, Y_temp, estimate_CI=True)
                # record epos for t_0, t_1
                if t_0 not in epos_dict: 
                    epos_dict[t_0] = [(mu_0, samples_0)]
                else:
                    epos_dict[t_0].append((mu_0, samples_0))
                if t_1 not in epos_dict: 
                    epos_dict[t_1] = [(mu_1, samples_1)]
                else:
                    epos_dict[t_1].append((mu_1, samples_1))
        ...
        if take_mean == "both":
            ...
            if not take_mean:
                ...
                if not alpha:
                    ...
                    epos_CIs = dict()
                    for t_val in epos_dict:
                        epos = [epo for epo, _ in epos_dict[t_val]]
                        list_of_samples = [samples for _, samples in epos_dict[t_val]]
                        epos_means[t_val] = np.mean(epos)
                        # Calculate CI 
                        all_samples = np.array(list_of_samples)
                        mean_samples = all_samples.mean(axis=0)
                        lower_bound, upper_bound = sample_confidence_interval(
                            torch.from_numpy(mean_samples).float(), 
                            alphas=torch.tensor([alpha]).float()
                        )
                        epos_CIs[t_val] = (lower_bound, upper_bound)
                    return epos_means, epos_CIs
    ...
    def _predict_mu_0_and_mu_1(
        ...
        model.fit(X, T, Y)
        if not estimate_CI: # don't estimate CI 
